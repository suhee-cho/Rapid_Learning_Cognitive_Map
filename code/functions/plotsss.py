...
# num_CA3_neurons = global_variables.num_CA3_neurons
# num_CA1_neurons = global_variables.num_CA1_neurons
num_IN_neurons = global_variables.num_IN_neurons
len_sim = global_variables.rest_time  # ms
num_state_total = global_variables.num_state_total
spike_th_PC = -3.25524288  # (re-optimized by Szabolcs)
spike_th_BC = -34.78853881  # (re-optimized by Szabolcs)

def plot_box(data1, data2, fig, ax, ylim=[-0.0,1.4]):
    # Plot the boxplots
    # 'positions' argument places the boxplots at specific x-coordinates
    ...
    # Connect corresponding dhata points
    # We assume a 1:1 correspondence for simplicity
    # for i in range(len(data1)):
    #     ax.plot([1, 2], [data1[i], data2[i]], 'k-', alpha=0.3) # 'k-' for black line

    # Set x-axis labels and title
    ax.set_xticks([1, 2])
    ax.set_xticklabels(['Group 1', 'Group 2'])
    ax.set_ylim(ylim)
    # ax.set_title('Boxplots with Connected Data Points')
...
def plot_shaded(xx,mean_val,error_val,color='black'):
    plt.fill_between(xx, mean_val-error_val, mean_val+error_val, alpha=0.2, color=color, linewidth=0)
    plt.plot(xx, mean_val, color=color)

def average_weight(w, n_pops):
    
    pop_size_x = int(w.shape[1] / n_pops)
    pop_size_y = int(w.shape[0] / n_pops)
...
def plot_avg_weight(w, n_pops, fig=None, ax=None, fig_title=None, unit_ticks=[True,True], minmax=None, log=False, colormap="inferno"):

    if fig is None: fig, ax = plt.subplots(1,1,figsize=(5,4))

    # x_place_cell_ID_list = generate_place_cell_ID_list(np.array(list(x_place_fields.keys()),dtype=int), np.array(list(x_place_fields.values())))
    # x_target_idx = reorder_neuron_idx(x_place_cell_ID_list,x_place_fields,target_state,include_cue=include_cue)
    # weightmx = w[:,x_target_idx]
    
    # if y_place_fields is None:
    #     y_place_cell_ID_list = x_place_cell_ID_list
    #     y_target_idx = x_target_idx
    # else:
    #     y_place_cell_ID_list = generate_place_cell_ID_list(np.array(list(y_place_fields.keys()),dtype=int), np.array(list(y_place_fields.values())))
    #     y_target_idx = reorder_neuron_idx(y_place_cell_ID_list,y_place_fields,target_state,include_cue=True)
    # weightmx = weightmx[y_target_idx,:]

    # Total number of datapoints (boxes)
    # n_pops = len(target_state)*pop_per_unit
    mean_wmx = average_weight(w, n_pops)

    x = np.arange(n_pops + 1)
    y = np.arange(n_pops + 1)
    if log:
        if minmax == None:
            i = ax.pcolormesh(x, y, mean_wmx, cmap=colormap, shading='auto', norm=LogNorm(vmin=mean_wmx.min(), vmax=mean_wmx.max()))#,norm=LogNorm())#
        else:
            i = ax.pcolormesh(x, y, mean_wmx, cmap=colormap, norm=LogNorm(vmin=minmax[0], vmax=minmax[1]),shading='auto')
    else:
        if minmax == None:
            i = ax.pcolormesh(x, y, mean_wmx, cmap=colormap, shading='auto')#,norm=LogNorm())#
        else:
            i = ax.pcolormesh(x, y, mean_wmx, cmap=colormap, vmin=minmax[0], vmax=minmax[1],shading='auto')
...
# def plot_raster(spike_times, spiking_neurons, rate, hist, slice_idx, color_):
#     """
...
#     fig.tight_layout()
#     fig_name = os.path.join(fig_dir, "raster_plot.png")
...
# def plot_PSD(rate, rate_ac, f, Pxx, title_, color_):
#     """
...
#     f = np.asarray(f)
#     f_ripple = f[np.where((110 < f) & (f < 200))]; Pxx_ripple_plot = Pxx_plot_mean[np.where((110 < f) & (f < 200))]
...
#     except:
#         pass
#     ax3.plot(f, Pxx_plot_mean, color=color_)
#     ax3.plot(f_ripple, Pxx_ripple_plot, "r-",linewidth=2, label="ripple (110-200 Hz)")
#     #ax3.plot(f_gamma, Pxx_gamma_plot, "k-", linewidth=2, label="gamma (30-100 Hz)")
...
# def plot_zoomed(spike_times, spiking_neurons, rate, title_, color_, PC_pop=True, StateM=None, selection=None):
#     """
...
#         ymax = spiking_neurons.max()+5 if spiking_neurons.max()+5 < num_CA3_neurons else num_CA3_neurons
#         subset = _select_subset(selection, ymin, ymax)
#         size_ = 12
#     else:
#         ymin = 0; ymax = num_IN_neurons
#         size_ = 20
...
#             id_ = int(num_IN_neurons/2)  # fixed in simulations
...
#     fig.tight_layout()
#     fig_name = os.path.join(fig_dir, "%s_zoomed.png" % title_)
#     fig.savefig(fig_name)
...
# def plot_detailed(StateM, subset, plot_adaptation=True):
#     """
...
#     sns.despine()
#     fig.tight_layout()
#     fig_name = os.path.join(fig_dir, "PC_population_zoomed_detailed.png")
#     fig.savefig(fig_name)


# def plot_LFP(t, LFP, f, Pxx):
#     """
...
#     except:
#         pass
#     ax2.plot(f, Pxx_plot_mean, color="black")
#     ax2.plot(f_ripple, Pxx_ripple_plot, "r-", linewidth=2, label="ripple (110-200 Hz)")
#     #ax2.plot(f_gamma, Pxx_gamma_plot, "k-", linewidth=1.5, label="gamma (30-100 Hz)")
...
#     sns.despine()
#     fig.tight_layout()
#     fig_name = os.path.join(fig_dir, "LFP.png")
#     fig.savefig(fig_name)
...
def plot_wmx(wmx):
    ...
    fig = plt.figure(figsize=(10, 8))
    ax = fig.add_subplot(1, 1, 1)
    i = ax.imshow(wmx, cmap="inferno", origin="lower", interpolation="nearest")  # nS conversion
    fig.colorbar(i)
...
#     assert num_CA3_neurons % n_pops == 0

#     pop_size = int(num_CA3_neurons / n_pops)
#     mean_wmx = np.zeros((n_pops, n_pops))
#     for i in range(n_pops):
#         for j in range(n_pops):
...
#     i = ax.imshow(mean_wmx, cmap="inferno", origin="lower", interpolation="nearest")
#     fig.colorbar(i)
...
# def plot_weights(incomming_weights):
#     """
...
def eSS_voltage(t, v, SS_voltage, current):
    """
